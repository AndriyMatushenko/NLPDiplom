...
def scrape_legal_data():
    """
    Завантажує та очищує тексти юридичних документів із джерел,
    визначених у константі `SEARCH_SOURCES`.

    Процес для кожного URL:
    1. Виконує GET-запит з імітацією браузера.
    2. Витягує весь текстовий вміст зі сторінки.
    3. Очищує текст від зайвих пробілів та переносів рядків.

    :return: Список словників, де кожен словник представляє
             один документ і має ключі 'source' (URL) та 'text' (очищений текст).
    :rtype: list[dict]
    """
    documents = []
...
class Search:
    ...
    Інкапсулює логіку завантаження попередньо оброблених даних,
    створення пошукових індексів (BM25, TF-IDF) та виконання запитів.

    :ivar original_chunks: Список оригінальних текстових фрагментів.
    :vartype original_chunks: list[dict]
    :ivar bm25: Об'єкт індексу BM25 для ранжування.
    :vartype bm25: rank_bm25.BM25Okapi
    """
    def __init__(self):
...
def generate_answer(context, query_text):
    """
    Генерує відповідь на запит користувача, використовуючи наданий
    контекст та велику мовну модель (LLM) через Ollama.

    Формує спеціальний промпт, який інструктує модель спиратися
    виключно на надану інформацію, щоб мінімізувати "галюцинації".

    :param context: Текстовий блок, знайдений пошуковою системою,
                    який є найбільш релевантним до запиту.
    :type context: str
    :param query_text: Оригінальний текстовий запит користувача.
    :type query_text: str
    :return: Згенерована моделлю відповідь. У випадку помилки повертає
